...
pub(crate) async fn search_tavily(
    client: &reqwest::Client,
    query: &str,
    max_results: usize,
    keys: &[String],
...
pub(crate) async fn search_firecrawl(
    client: &reqwest::Client,
    query: &str,
    max_results: usize,
    keys: &[String],
...
pub(crate) async fn search_anysearch(
    client: &reqwest::Client,
    query: &str,
    max_results: usize,
    keys: &[String],
...
pub(crate) const EXA_MCP_ENDPOINT: &str = "https://mcp.exa.ai/mcp";

/// 免 key 通道的 MCP 端点，可用 GQY_EXA_MCP_ENDPOINT 覆盖（自建代理/测试用）
pub(crate) fn exa_mcp_endpoint() -> String {
...
pub(crate) async fn search_exa_public(
    client: &reqwest::Client,
    query: &str,
    max_results: usize,
) -> Result<String> {
...
pub(crate) fn exa_public_results(text: &str, max_results: usize) -> Vec<Value> {
    text.split("\n---\n")
        .filter_map(|block| {
...
pub(crate) async fn parse_mcp_body(response: reqwest::Response) -> Result<Value> {
...
pub(crate) fn mcp_content_text(result: &Value) -> String {
    result
        .get("content")
        .and_then(Value::as_array)
        .map(|items| {
...
pub(crate) fn is_ddg_blocked() -> bool {
    DDG_BLOCKED_UNTIL
        .lock()
        .ok()
...
pub(crate) fn set_ddg_blocked(duration: Duration) {
    if let Ok(mut guard) = DDG_BLOCKED_UNTIL.lock() {
        *guard = Some(Instant::now() + duration);
    }
}

pub(crate) fn is_sogou_blocked() -> bool {
    SOGOU_BLOCKED_UNTIL
        .lock()
        .ok()
        .and_then(|guard| guard.filter(|&t| t > Instant::now()))
...
pub(crate) fn set_sogou_blocked(duration: Duration) {
    if let Ok(mut guard) = SOGOU_BLOCKED_UNTIL.lock() {
        *guard = Some(Instant::now() + duration);
    }
}

pub(crate) fn looks_like_ddg_challenge(status: u16, html: &str) -> bool {
...
pub(crate) fn is_result_url_allowed(url: &str) -> bool {
...
pub(crate) fn dedupe_key(url: &str) -> String {
    let lower = url.to_lowercase();
...
pub(crate) fn unwrap_ddg_url(url: &str) -> String {
...
pub(crate) fn unwrap_yahoo_url(url: &str) -> String {
    let url = html_unescape(url.trim());
...
pub(crate) fn extract_snippet_after(text: &str, marker: &str) -> Option<String> {
