...
pub(crate) fn auto_configure_model_tags(paths: &GQYPaths, provider: &mut ProviderConfig, model: &str) {
...
pub(crate) fn edit_embedding_advanced(stdout: &mut io::Stdout, config: &mut AppConfig) -> Result<()> {
    let mut fields = vec![
        Field::new(
...
pub(crate) fn edit_platform_command_prefix(stdout: &mut io::Stdout, config: &mut AppConfig) -> Result<()> {
...
pub(crate) fn platform_command_permission_label(permission: PlatformCommandPermission) -> &'static str {
    match permission {
...
pub(crate) fn select_platform_commands(stdout: &mut io::Stdout, config: &mut AppConfig) -> Result<()> {
    let mut selected = 0usize;
    loop {
...
pub(crate) fn edit_qq(stdout: &mut io::Stdout, paths: &GQYPaths, config: &mut AppConfig) -> Result<()> {
    let mut selected = 0usize;
    loop {
...
pub(crate) fn edit_platform_rate_limit(stdout: &mut io::Stdout, limit: &mut PlatformRateLimit) -> Result<()> {
    let mut fields = vec![
        Field::new(
            t(
